...
$probeSlug = 'windows-remove-item-void-probe'
...
$removeItemVoidRva = if ($env:BML_TEST_REMOVE_ITEM_VOID_RVA) { $env:BML_TEST_REMOVE_ITEM_VOID_RVA } else { '3045456' }
...
$env:BML_REMOVE_ITEM_VOID_CHEST_PROBE_RVA = [string]$removeItemVoidRva
...
$probe = Get-Content -Raw (Join-Path $reportRoot 'windows-remove-item-void-chest-probe-report.json') | ConvertFrom-Json
$runtime = Get-Content -Raw (Join-Path $reportRoot 'runtime-load-report.json') | ConvertFrom-Json

if ($probe.status -ne 'installed' -or -not $probe.codeViewMatch -or $probe.rva -ne 3045456 -or -not $probe.prologueMatch -or -not $probe.installed -or $probe.replacementCalls -ne 0) {
...
Write-Host 'OK: Windows removeItemFromVoidChestServer install-only probe passed.'
