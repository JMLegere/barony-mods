...
$probeSlug = 'windows-add-item-void-probe'
...
$addItemVoidRva = if ($env:BML_TEST_ADD_ITEM_VOID_RVA) { $env:BML_TEST_ADD_ITEM_VOID_RVA } else { '3031968' }
...
$env:BML_ADD_ITEM_VOID_CHEST_PROBE_RVA = [string]$addItemVoidRva
...
$probe = Get-Content -Raw (Join-Path $reportRoot 'windows-add-item-void-chest-probe-report.json') | ConvertFrom-Json
$runtime = Get-Content -Raw (Join-Path $reportRoot 'runtime-load-report.json') | ConvertFrom-Json

if ($probe.status -ne 'installed' -or -not $probe.codeViewMatch -or $probe.rva -ne 3031968 -or -not $probe.prologueMatch -or -not $probe.installed -or $probe.replacementCalls -ne 0) {
...
Write-Host 'OK: Windows addItemToVoidChestServer install-only probe passed.'
