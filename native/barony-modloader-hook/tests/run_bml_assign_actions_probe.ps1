...
$probeSlug = 'windows-assign-actions-probe'
...
$expectedMapName = if ($env:BML_TEST_EXPECTED_MAP_NAME) { $env:BML_TEST_EXPECTED_MAP_NAME } elseif ($mapName) { $mapName } else { $null }
$expectedReturnRva = $env:BML_TEST_EXPECTED_RETURN_RVA
$assignRva = $env:BML_TEST_ASSIGN_ACTIONS_RVA
if (-not [Environment]::Is64BitProcess) {
    Write-Host 'SKIP: 64-bit PowerShell is required for the assignActions probe.'
    exit 0
}
...
if (-not $assignRva) { Write-Host 'SKIP: BML_TEST_ASSIGN_ACTIONS_RVA is not set for the assignActions fired-hook probe.'; exit 0 }
...
[Environment]::SetEnvironmentVariable('BML_INIT_CLASS_PROBE_EXIT_ON_FIRE', $null, 'Process')
[Environment]::SetEnvironmentVariable('BML_STASH_INSTALL_GET_ITEM_PASSTHROUGH', $null, 'Process')
[Environment]::SetEnvironmentVariable('BML_ASSIGN_ACTIONS_PROBE_RVA', [string]$assignRva, 'Process')
$assignExitOnFire = if ($expectedMapName) { '0' } else { '1' }
[Environment]::SetEnvironmentVariable('BML_ASSIGN_ACTIONS_PROBE_EXIT_ON_FIRE', $assignExitOnFire, 'Process')
...
$screenshotPath = Join-Path $reportRoot 'assignactions-probe-timeout.png'
$probePath = Join-Path $reportRoot 'windows-assign-actions-probe-report.json'
$fired = $false
for ($i = 0; $i -lt 30; ++$i) {
    Start-Sleep -Seconds 1
    if (Test-Path $probePath) {
        $probe = Get-Content -Raw $probePath | ConvertFrom-Json
        if ($probe.status -eq 'fired' -and $probe.replacementCalls -gt 0) {
            $returnMatches = $true
            if ($expectedReturnRva) {
                $returnMatches = ([string]$probe.lastReturnRva -eq [string]$expectedReturnRva)
            }
            $mapMatchesGlobal = $probe.map -and ([int64]$probe.map.globalMapSymbol -gt 0) -and $probe.map.argumentMatchesGlobal
            if ($expectedMapName) {
                $actualMapName = ''
                if ($probe.map -and $probe.map.name) { $actualMapName = [string]$probe.map.name }
                if ($returnMatches -and $mapMatchesGlobal -and $actualMapName.ToLowerInvariant() -eq $expectedMapName.ToLowerInvariant()) {
                    $fired = $true
                    break
                }
            } elseif ($returnMatches -and $mapMatchesGlobal) {
                $fired = $true
                break
            }
        }
...
Write-Host ("launcherAliveAtTimeout=" + (-not $launcherProcess.HasExited))
if ($launcherProcess.HasExited) { Write-Host ("launcherExitCode=" + $launcherProcess.ExitCode) }
if (Test-Path $screenshotPath) { Write-Host ("timeoutScreenshot=" + $screenshotPath) }
if (-not (Test-Path $probePath)) { throw 'assignActions probe report was not written' }
if (-not $fired) {
    $probe = Get-Content -Raw $probePath | ConvertFrom-Json
    throw "assignActions fired-hook probe did not fire within timeout: $($probe | ConvertTo-Json -Compress)"
}
...
if ($probe.status -ne 'fired' -or $probe.replacementCalls -le 0 -or -not $probe.installed) {
    throw "assignActions fired-hook probe validation failed: $($probe | ConvertTo-Json -Compress)"
}
if (-not $probe.map -or [int64]$probe.map.globalMapSymbol -le 0 -or -not $probe.map.argumentMatchesGlobal) {
    throw "assignActions probe did not validate the resolved global map symbol: $($probe | ConvertTo-Json -Compress)"
}
if ($expectedMapName) {
    $actualMapName = ''
    if ($probe.map -and $probe.map.name) { $actualMapName = [string]$probe.map.name }
    if ($actualMapName.ToLowerInvariant() -ne $expectedMapName.ToLowerInvariant()) {
        throw "assignActions did not reach expected map '$expectedMapName': $($probe | ConvertTo-Json -Compress)"
    }
}
...
Write-Host 'OK: Windows assignActions fired-hook probe passed.'
