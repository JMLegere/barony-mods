...
$probeSlug = 'windows-get-item-probe'
...
$env:BML_STASH_INSTALL_GET_ITEM_PASSTHROUGH = '1'
...
$probe = Get-Content -Raw (Join-Path $reportRoot 'windows-get-item-passthrough-install-report.json') | ConvertFrom-Json
$runtime = Get-Content -Raw (Join-Path $reportRoot 'runtime-load-report.json') | ConvertFrom-Json

if ($probe.status -ne 'installed' -or -not $probe.codeViewMatch -or $probe.rva -ne 3044448 -or -not $probe.prologueMatch -or -not $probe.installed -or $probe.replacementCalls -ne 0) {
...
Write-Host 'OK: Windows getItemFromChest install-only probe passed.'
