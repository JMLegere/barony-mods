...
$probeSlug = 'windows-new-entity-probe'
...
$newEntityRva = $env:BML_TEST_NEW_ENTITY_RVA
if (-not [Environment]::Is64BitProcess) {
    Write-Host 'SKIP: 64-bit PowerShell is required for the newEntity probe.'
    exit 0
}
...
if (-not $newEntityRva) { Write-Host 'SKIP: BML_TEST_NEW_ENTITY_RVA is not set for the newEntity fired-hook probe.'; exit 0 }
...
[Environment]::SetEnvironmentVariable('BML_NEW_ENTITY_PROBE_RVA', [string]$newEntityRva, 'Process')
[Environment]::SetEnvironmentVariable('BML_NEW_ENTITY_PROBE_EXIT_ON_FIRE', '1', 'Process')
...
$probePath = Join-Path $reportRoot 'windows-new-entity-probe-report.json'
$fired = $false
for ($i = 0; $i -lt 30; ++$i) {
    Start-Sleep -Seconds 1
    if (Test-Path $probePath) {
        $probe = Get-Content -Raw $probePath | ConvertFrom-Json
        if ($probe.status -eq 'fired' -and $probe.replacementCalls -gt 0 -and $probe.installed) {
            $fired = $true
            break
        }
    }
...
if (-not (Test-Path $probePath)) { throw 'newEntity probe report was not written' }
if (-not $fired) {
    $probe = Get-Content -Raw $probePath | ConvertFrom-Json
    throw "newEntity fired-hook probe did not fire within timeout: $($probe | ConvertTo-Json -Compress)"
}
...
if ($probe.status -ne 'fired' -or $probe.replacementCalls -le 0 -or -not $probe.installed) {
    throw "newEntity fired-hook probe validation failed: $($probe | ConvertTo-Json -Compress)"
}
...
Write-Host 'OK: Windows newEntity fired-hook probe passed.'
