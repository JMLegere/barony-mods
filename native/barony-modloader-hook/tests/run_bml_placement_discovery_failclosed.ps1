...
$probeSlug = 'windows-placement-failclosed'
...
if (-not [Environment]::Is64BitProcess) { Write-Host 'SKIP: 64-bit PowerShell is required for the placement failclosed probe.'; exit 0 }
...
$env:BML_STASH_PLACEMENT_DISCOVERY = '1'
...
if ($LASTEXITCODE -eq 0) { throw 'launch unexpectedly succeeded' }
...
if ($runtime.status -ne 'failed') {
    throw "Expected failed runtime report: $($runtime | ConvertTo-Json -Compress)"
}
$codes = @($runtime.errors | ForEach-Object { $_.code })
if ($codes -notcontains 'BML_WINDOWS_PLACEMENT_DISCOVERY_INSTALL_FAILED') {
    throw "Expected placement failclosed code: $($runtime | ConvertTo-Json -Compress)"
}
if (Test-Path (Join-Path $reportRoot 'stash-placement-discovery-report.json')) {
    throw 'Placement discovery report should not be written when installation fails.'
}

Write-Host 'OK: Windows placement discovery fails closed without RVAs.'
