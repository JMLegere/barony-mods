...
$probeSlug = 'windows-set-sprite-probe'
...
$setSpriteRva = $env:BML_SET_SPRITE_PROBE_RVA

if (-not [Environment]::Is64BitProcess) { Write-Host 'SKIP: 64-bit PowerShell is required for the setSprite probe.'; exit 0 }
...
if (-not $setSpriteRva) { Write-Host 'SKIP: BML_SET_SPRITE_PROBE_RVA is not set for the setSprite fired-hook probe.'; exit 0 }
...
[Environment]::SetEnvironmentVariable('BML_SET_SPRITE_PROBE_RVA', [string]$setSpriteRva, 'Process')
...
[Environment]::SetEnvironmentVariable('BML_SET_SPRITE_PROBE_EXIT_ON_FIRE', '1', 'Process')
...
$probePath = Join-Path $reportRoot 'windows-set-sprite-probe-report.json'
$fired = $false
for ($i = 0; $i -lt 30; ++$i) {
...
if (-not (Test-Path $probePath)) { throw 'setSprite probe report was not written' }
$probe = Get-Content -Raw $probePath | ConvertFrom-Json
if (-not $fired) {
    throw "setSprite fired-hook probe did not fire within timeout: $($probe | ConvertTo-Json -Compress)"
}
...
Write-Host 'OK: Windows setSprite fired-hook probe passed.'
