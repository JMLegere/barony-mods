...
| `stash_lobby_placement` | 8 | 0 | Lobby placement/prompt target prologues, including `uidToEntity`, are patch-window-ready after relative branch and supported ModRM relocation support. |
| `stash_shop_placement` | 10 | 0 | Shop placement/prompt target prologues, including `uidToEntity`, are patch-window-ready after pxor/lea, relative branch, and supported ModRM relocation support. |
...
The scan is intentionally conservative and byte-level. Data symbols can be ready because no prologue detour is required. A function target marked `ready` only means the current backend can reserve and relocate an absolute-jump patch window for that prologue; it is not a claim that every player-facing Stash scenario is verified. The current Steam/Linux static scan has no missing or blocked required Stash targets; the hook now has opt-in pass-through install reports for `Entity::addItemToVoidChestServer` alone, the seven-target core inventory lifecycle set, and the eight-target access/placement/prompt set, a fake-provider access/placement/prompt call-through self-test, plus a fake-provider behavior self-test. `BML_STASH_ENABLE_EXPERIMENTAL_CORE_BEHAVIOR=1` with `BML_STASH_CORE_BEHAVIOR_SELF_TEST=1` writes `BaronyModLoader/reports/stash-core-behavior-report.json` and validates only the state-backed core lifecycle in the fake provider: loading one seeded item from `BaronyModLoader/state/stash-inventory-v1.tsv`, routing `Entity::addItemToVoidChestServer` through the bound `stats[0]->void_chest_inventory` list, exercising generic `Entity::addItemToChest` and `Entity::getItemFromChest` add/remove mutation against that same list, and saving two rows on close. Production activation now adds `uidToEntity` and `Language::get` for the scoped hover prompt and loads Stash for validated manifests; remaining player-facing proof is tracked in the deliverables file.
