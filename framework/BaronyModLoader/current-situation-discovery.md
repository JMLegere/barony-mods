...
Option C is the current Stash safety boundary: required gameplay hooks are represented as `hookTargets` tied to Stash capabilities, and the native hook analyzes those direct Stash targets through the abstract `linux-x86_64-direct-stash-detour` backend. The backend is still `analyze-only`: it reports data targets as `ready`, uses the conservative detour decoder to mark function prologues as patch-window-ready, records `patchWindowBytes`, and requires unresolved symbols, blocked detour targets, or uninstalled required hook groups to prevent `runtime-load-report.json` from claiming Stash loaded. Separate opt-in bridges can install pass-through detours without changing that Stash load decision: `BML_STASH_INSTALL_ADD_ITEM_PASSTHROUGH=1` reports the single `Entity::addItemToVoidChestServer` bridge, `BML_STASH_INSTALL_CORE_PASSTHROUGH=1` reports the seven-target core inventory lifecycle bridge for `Entity::getChestInventoryList`, `Entity::addItemToChest`, `Entity::getItemFromChest`, `Entity::addItemToVoidChestServer`, `Entity::removeItemFromVoidChestServer`, `Entity::closeChest`, and `Entity::closeChestServer`, and `BML_STASH_INSTALL_ACCESS_PLACEMENT_PASSTHROUGH=1` reports the seven-target access/placement/prompt bridge for `actChest`, `actChestLid`, `generateDungeon`, `assignActions`, `newEntity`, `setSpriteAttributes`, and `Language::get`. `BML_STASH_ACCESS_PLACEMENT_SELF_TEST=1` adds a fake-provider-only call-through check for those seven replacements, including scoped `Open stash` prompt replacement, and the access/placement install report is rewritten at process exit so timed stock-game probes can record live replacement call counts. The relocator now handles the installed `Entity::addItemToChest` thunk shape where a copied prologue branches backward to an out-of-line `.part.0` body. The hook also skips inherited `LD_PRELOAD` in non-Barony helper processes unless `BML_HOOK_ALLOW_NON_BARONY=1` is set for fake-provider tests. These reports are deliberately outside `stash-hook-report.json` and do not claim playable behavior.
...
| --- | ---: | ---: | --- |
| `stash_void_chest_binding` | 5 | 0 | `actChest`, `actChestLid`, `Entity::getChestInventoryList`, `Entity::addItemToVoidChestServer`, and `Entity::removeItemFromVoidChestServer` have decoder-safe patch windows after RIP-relative ModRM relocation support. |
| `stash_inventory_persistence` | 11 | 0 | The `stats` data symbol, getter/add/get/close chest paths including the terminal-return `Entity::closeChestServer` prologue, `newItem`, and the list helper prologues are patch-window-ready; no gameplay hook is installed yet. |
| `stash_lobby_placement` | 7 | 0 | All current lobby placement target prologues are patch-window-ready after relative branch relocation support, but no gameplay hook is installed yet. |
| `stash_shop_placement` | 9 | 0 | All current shop placement target prologues are patch-window-ready after pxor/lea and relative branch support, but no gameplay hook is installed yet. |
...
The scan is intentionally conservative and byte-level. Data symbols can be ready because no prologue detour is required. A function target marked `ready` only means the current backend can reserve and relocate an absolute-jump patch window for that prologue; it is not a claim that Stash gameplay detours are installed. The current Steam/Linux static scan has no missing or blocked required Stash targets; the hook now has opt-in pass-through install reports for `Entity::addItemToVoidChestServer` alone, the seven-target core inventory lifecycle set, and the seven-target access/placement/prompt set, a fake-provider access/placement/prompt call-through self-test, plus an experimental fake-provider behavior self-test. `BML_STASH_ENABLE_EXPERIMENTAL_CORE_BEHAVIOR=1` with `BML_STASH_CORE_BEHAVIOR_SELF_TEST=1` writes `BaronyModLoader/reports/stash-core-behavior-report.json` and validates only the state-backed core lifecycle in the fake provider: loading one seeded item from `BaronyModLoader/state/stash-inventory-v1.tsv`, routing `Entity::addItemToVoidChestServer` through the bound `stats[0]->void_chest_inventory` list, exercising generic `Entity::addItemToChest` and `Entity::getItemFromChest` add/remove mutation against that same list, and saving two rows on close. A separate timed installed Steam/Linux probe with `BML_STASH_ENABLE_EXPERIMENTAL_CORE_BEHAVIOR=1`, `BML_STASH_INSTALL_ACCESS_PLACEMENT_PASSTHROUGH=1`, and no fake self-test reaches the stock game loop without preloading non-Barony helper processes, writes the core behavior report with all seven core target detours installed, wrote `BaronyModLoader/reports/stash-access-placement-detour-install-report.json` with all six then-current access/placement target detours installed, rewrote that access/placement report at process exit with live replacement calls for `assignActions`, `newEntity`, and `setSpriteAttributes`, and still failed runtime loading only with `BML_STASH_HOOKS_NOT_INSTALLED`. Newer production activation now adds `Language::get` for the scoped prompt and loads Stash for validated manifests; remaining player-facing proof is tracked in the deliverables file.
...
5. Treat Stash `hookTargets` and `stash-hook-report.json` as the direct Stash hook readiness report and fail-closed gameplay hook scaffold until the complete behavior-changing detour set is installed and verified.
