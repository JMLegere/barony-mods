...
Current Steam/Linux conservative direct-target scan for build `22630456`, generated by `python native/barony-modloader-hook/tools/analyze_stash_targets.py --manifest native/barony-modloader-hook/manifests/steam-371970-22630456-linux.json --executable /home/jerry/.local/share/Steam/steamapps/common/Barony/barony.x86_64`, using the decoder support implemented through the REX/register/non-RIP-memory and relative-control-flow relocation slices:

| Hook group | Ready targets | Blocked targets | Interpretation |
| --- | ---: | ---: | --- |
| `stash_void_chest_binding` | 3 | 2 | `actChestLid`, `Entity::addItemToVoidChestServer`, and `Entity::removeItemFromVoidChestServer` have decoder-safe patch windows; `actChest` still needs more instruction decoding and `Entity::getChestInventoryList` still needs RIP-relative relocation. |
| `stash_inventory_persistence` | 7 | 4 | The `stats` data symbol, `Entity::getItemFromChest`, `newItem`, and the list helper prologues are ready; getter/add/close paths still need RIP-relative, movzx/multi-byte, or early-return-aware relocation support. |
...
| `stash_shop_placement` | 8 | 1 | Data symbols plus `assignActions`, `newEntity`, and `setSpriteAttributes` are ready; `generateDungeon` still needs broader instruction decoding. |
