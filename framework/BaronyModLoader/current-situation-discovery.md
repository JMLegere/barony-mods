...
The scan is intentionally conservative and byte-level. Data symbols can be ready because no prologue detour is required. A function target marked `ready` only means the current backend can reserve and relocate an absolute-jump patch window for that prologue; it is not a claim that Stash gameplay detours are installed. The current Steam/Linux static scan has no missing or blocked required Stash targets; the hook now has opt-in pass-through install reports for both `Entity::addItemToVoidChestServer` alone and the seven-target core inventory lifecycle set, plus an experimental fake-provider behavior self-test. `BML_STASH_ENABLE_EXPERIMENTAL_CORE_BEHAVIOR=1` with `BML_STASH_CORE_BEHAVIOR_SELF_TEST=1` writes `BaronyModLoader/reports/stash-core-behavior-report.json` and validates only the state-backed core lifecycle in the fake provider: loading one seeded item from `BaronyModLoader/state/stash-inventory-v1.tsv`, routing `Entity::addItemToVoidChestServer` through the bound `stats[0]->void_chest_inventory` list, exercising generic `Entity::addItemToChest` and `Entity::getItemFromChest` add/remove mutation against that same list, and saving two rows on close. A separate timed installed Steam/Linux probe with `BML_STASH_ENABLE_EXPERIMENTAL_CORE_BEHAVIOR=1` and no fake self-test now reaches the stock game loop, writes the same behavior report with all seven target detours installed, and still fails runtime loading only with `BML_STASH_HOOKS_NOT_INSTALLED`. That is not an in-game or playable Stash claim; runtime loading still fails closed while the full gameplay hook set is incomplete.
